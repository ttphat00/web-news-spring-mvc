		...
		<form action="<c:url value='/quan-tri/bai-viet/danh-sach'/>" id="formSubmit" method="get">
			...
			<div class="main-content-inner">
				<div class="breadcrumbs ace-save-state" id="breadcrumbs">
					<ul class="breadcrumb">
						<li>
							<i class="ace-icon fa fa-home home-icon"></i> <a href="#">Trang chủ</a>
						</li>
					</ul>
					<!-- /.breadcrumb -->
				</div>
											...
											<tbody>
												<c:forEach var="item" items="${model.listResult}">
													<tr>
														...
														<td style="width: 20%"><img alt="" src="${item.thumbnail}" style="width: 100%"></td>
														<td>${item.title}</td>
	...
	<script>
		...
		function warningBeforeDelete(){
			swal({
			  ...
			  if (isConfirm.value) {
				  var ids = $('tbody input[type=checkbox]:checked').map(function (index, item) {
		...
		$('#checkAll').change(function(){
			if(this.checked){
				$('tbody input[type=checkbox]').map(function (index, item) {
					  item.checked = true;
				});
			}else{
				$('tbody input[type=checkbox]').map(function (index, item) {
